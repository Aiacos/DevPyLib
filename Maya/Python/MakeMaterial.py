...
def connect_specularColor(shader, path, ShaderName, specularColor, outSocket, gammaCorrect=False, singlePlaceNode=False):
    #creation node
    try:
        file_node_specularColor = cmds.shadingNode("file",asTexture=True, isColorManaged=True)
        cmds.setAttr(file_node_specularColor+'.fileTextureName', path+shaderName+specularColor, type='string')
...
def connect_specularWeight(shader, path, ShaderName, specularWeight, outSocket, outValue=1, gammaCorrect=False, singlePlaceNode=False):
    #creation node
    try:
        file_node_specularWeight = cmds.shadingNode("file",asTexture=True, isColorManaged=True)
        cmds.setAttr(file_node_specularWeight+'.fileTextureName', path+shaderName+specularWeight, type='string')
...
def connect_specularRoughness(shader, path, ShaderName, specularRoughness, outSocket, gammaCorrect=False, singlePlaceNode=False):
    #creation node
    file_node_specularRoughness = cmds.shadingNode("file",asTexture=True, isColorManaged=True)
    cmds.setAttr(file_node_specularRoughness+'.fileTextureName', path+shaderName+specularRoughness, type='string')
    cmds.setAttr(file_node_specularRoughness+'.alphaIsLuminance', True)
...
def connect_fresnel(shader, path, ShaderName, fresnel, outSocket, outValue=0.039, gammaCorrect=False, singlePlaceNode=False):
    #creation node
    try:
        file_node_fresnel = cmds.shadingNode("file",asTexture=True, isColorManaged=True)
        cmds.setAttr(file_node_fresnel+'.fileTextureName', path+shaderName+fresnel, type='string')
...
def makeAiStandard(path, ShaderName, singlePlaceNode=True):
    ...
    connect_backlighting(shader, path, ShaderName, backlighting, outSocket='Kb', gammaCorrect=False, singlePlaceNode=place_node)
    connect_specularColor(shader, path, ShaderName, specularColor, outSocket='KsColor', gammaCorrect=True, singlePlaceNode=place_node)
    connect_specularWeight(shader, path, ShaderName, specularWeight, outSocket='Ks', gammaCorrect=False, singlePlaceNode=place_node)
    connect_specularRoughness(shader, path, ShaderName, specularRoughness, outSocket='specularRoughness', gammaCorrect=False, singlePlaceNode=place_node)
    connect_fresnel(shader, path, ShaderName, fresnel, outSocket='Ksn', gammaCorrect=False, singlePlaceNode=place_node)
    connect_normal(shader, path, ShaderName, normal, outSocket='normalCamera', gammaCorrect=False, singlePlaceNode=place_node)
...
def makeMia_Material_X(path, ShaderName, singlePlaceNode=True):## Not tested
    ...
    ####
    connect_diffuse(shader, path, ShaderName, diffuse, outSocket='diffuse', gammaCorrect=False, singlePlaceNode=place_node)#ok
    connect_backlighting(shader, path, ShaderName, backlighting, outSocket='Kb', gammaCorrect=False, singlePlaceNode=place_node)#non esiste
    connect_specularColor(shader, path, ShaderName, specularColor, outSocket='refl_color', gammaCorrect=False, singlePlaceNode=place_node)#ok
    connect_specularWeight(shader, path, ShaderName, specularWeight, outSocket='reflectivity', gammaCorrect=False, singlePlaceNode=place_node)#ok
    connect_specularRoughness(shader, path, ShaderName, specularRoughness, outSocket='refl_gloss', gammaCorrect=False, singlePlaceNode=place_node)#ok
    connect_fresnel(shader, path, ShaderName, fresnel, outSocket='brdf_0_degree_refl', gammaCorrect=False, singlePlaceNode=place_node)#fallirebbe, check non presente
    connect_normal(shader, path, ShaderName, normal, outSocket='standard_bump', gammaCorrect=False, singlePlaceNode=place_node)#ok

...
def create_all_control_shapes():
    ...
    for idx, shape_name in enumerate(shapes):
        ...
        try:
            ...
            # Add text annotation
            pm.annotate(ctrl.C, text=shape_name, point=(x_pos, 0, z_pos))
...
def test_shape_scaling():
    ...
    for idx, scale in enumerate(scales):
        try:
            ...
            # Add annotation
            pm.annotate(ctrl.C, text=f"scale={scale}", point=(idx * spacing, 0, -spacing))
...
def test_shape_direct_functions():
    ...
    for idx, (func_name, func) in enumerate(direct_shapes):
        try:
            ...
            pm.annotate(shape_obj, text=f"direct_{func_name}", point=(idx * spacing, 0, z_offset))

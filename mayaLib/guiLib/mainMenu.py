...
class SearchLineEdit(QtWidgets.QLineEdit):
    ...
    def __init__(self, icon_file, parent=None):
        ...
        self.setMinimumSize(
            max(
                self.minimumSizeHint().width(), buttonSize.width() + frameWidth * 2 + 3
            ),
            max(
                self.minimumSizeHint().height(),
                buttonSize.height() + frameWidth * 2 + 3,
            ),
        )
...
class MenuLibWidget(QtWidgets.QWidget):
    ...
    def __init__(self, libPath, parent=None):
        ...
        self.searchLineEdit.speak.connect(
            lambda: self.buildButtonList(self.searchLineEdit.text())
        )
...
class MainMenu(QtWidgets.QWidget):
    ...
    def __init__(
        self, libPath, menuName="MayaLib", parent=None, auto_update_on_load=True
    ):
        ...
        if menuName not in [m.text for m in self.mayaMenu.actions()]:
            ...
            QObject.connect(
                self.libWindow,
                SIGNAL("updateWidget()"),
                lambda: self.updateWidget(libPath),
            )

...
def clear_ncloth(arg=None):
    ...
    for each in sel:
        ...
        if any("blendShape" in x for x in mc.listConnections(mc.listRelatives(each, c=True))):
            bs_delete = mc.listConnections(
                mc.listConnections(mc.listRelatives(each, c=True), type="blendShape")[0]
                + ".inputTarget"
            )
...
def make_shaders():
    ...
    mc.setAttr(shader_name2 + ".color", 1, 0.5, 0.5)
...
def move_along_normal(arg=None):
    ...
    sel = mc.ls(sl=True, o=True, fl=True)
    for each in sel:
        mc.polyMoveVertex(each, ch=0, ran=0, ltz=0.01)
        mc.DeleteHistory()
    mc.select(sel)
...
def paint_weight(arg=None):
    ...
    """
    if (
        mc.listConnections(mc.listRelatives(mc.ls(sl=True, fl=True))[0], type="blendShape")
        is not None
    ):
...
def muscle_tool(arg=None):
    ...
    mc.rowLayout(nc=3)
    mc.button(l="Make Rigid", w=100, bgc=(0, 0.3, 0.2), c=make_rigid)
    mc.button(l="Remove nRigid", w=100, bgc=(0.3, 0, 0), c=clear_nrigid)
    mc.button(l="Select Rigids", w=100, c=select_nrigid)
    mc.setParent("..")
    mc.rowLayout(nc=3)
    mc.button(l="Make Muscle", w=100, bgc=(0, 0.3, 0.2), c=make_muscle)
    mc.button(l="Remove nCloth", w=100, bgc=(0.3, 0, 0), c=clear_ncloth)
    ...
    mc.rowLayout(nc=2)
    mc.floatField("clothThickness", v=0.01, w=40, pre=3, min=0, cc=apply_values)
    mc.text(l="Thickness")
    mc.setParent("..")
    ...
    mc.rowLayout(nc=2)
    mc.floatField("rigidThickness", v=0.01, w=40, pre=3, min=0, cc=apply_values_rigid)
    ...
    mc.text(l="-------------Freeze-------------")
    mc.rowLayout(nc=2)
    mc.button(l=" BlendShape ", bgc=(0.1, 0.4, 0.4), c=freeze_setup)
    mc.button(l=" Paint Weight ", bgc=(0.1, 0.3, 0.3), c=paint_weight)
    mc.setParent("..")
    mc.rowLayout(nc=1)
    mc.button(l=" Transform Constraint", bgc=(0, 0.3, 0.2), c=transform_constraint)
    mc.setParent("..")
    mc.rowLayout(nc=1)
    mc.button(l=" Remove Constraint", bgc=(0.3, 0, 0), c=remove_constraint)
    mc.setParent("..")
    ...
    mc.button(l="NEXT FRAME >", bgc=(0, 0.5, 0.3), c=time_next)
    mc.setParent("..")
    mc.rowLayout(nc=2)
    mc.button("playButton", l=" PLAY >> ", bgc=(0, 0.3, 0.2), c=time_play)
    mc.button(l=" > STOP < ", bgc=(0.3, 0, 0), c=time_stop)
    mc.setParent("..")

    mc.columnLayout()
    mc.text(l="-------------Shapes-------------")
    mc.rowLayout(nc=1)
    mc.button(l="Toggle Input/nCloth Mesh", bgc=(0.1, 0.3, 0.3), c=toggle_io)
    mc.setParent("..")
    mc.rowLayout(nc=1)
    mc.button(l="Delete Intermediate Object", bgc=(0.3, 0, 0), c=delete_io)
    mc.setParent("..")
    mc.rowLayout(nc=1)
    mc.button(l="Fix Shape Name", bgc=(0.2, 0.2, 0.2), c=fix_shape_name)
    mc.setParent("..")

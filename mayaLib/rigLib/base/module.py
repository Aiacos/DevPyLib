...
class Base:  # pylint: disable=too-many-instance-attributes,too-few-public-methods
    ...
    def __init__(  # pylint: disable=too-many-arguments,too-many-locals,too-many-statements
        ...
        **legacy_kwargs: Any,
        ...
        if legacy_kwargs:
            raise ValueError(f"Unexpected arguments for Base: {tuple(legacy_kwargs.keys())}")
        ...
        self.fast_model_group = pm.group(n="fastModel_GRP", em=True, p=self.model_group)
        self.medium_model_group = pm.group(n="mediumModel_GRP", em=True, p=self.model_group)
        self.medium_slow_group = pm.group(n="mediumSlowModel_GRP", em=True, p=self.model_group)
        self.slow_model_group = pm.group(n="slowModel_GRP", em=True, p=self.model_group)
        self.all_model_group = pm.group(n="allModel_GRP", em=True, p=self.model_group)
        self.rig_model_group = pm.group(n="rigModel_GRP", em=True, p=self.model_group)
        pm.hide(self.rig_model_group)

        self.rig_group = pm.group(n="rig_GRP", em=True, p=self.main_control.get_control())
        ...
        self.joints_group = pm.group(n="skeleton_GRP", em=True, p=self.main_control.get_control())
        self.modules_group = pm.group(n="modules_GRP", em=True, p=self.main_control.get_control())
        ...
        for attr_name, obj, default in zip(
            ...
            pm.setAttr(f"{self.global_control.get_control()}.{attr_name}", cb=True)
            pm.connectAttr(
        ...
        pm.setAttr(f"{self.global_control.get_control()}.{display_level_attr}", cb=True)
    ...
    def create_halo(self, attach_node: pm.PyNode | None, scale: float) -> None:
        ...
        self.halo_control.get_modify_grp().translateY.set(6 * self.halo_control.get_ctrl_scale())

    def create_display(self, attach_node: pm.PyNode | None, scale: float) -> control.Control:
        ...
        if attach_node:
            ...
            display_ctrl.get_modify_grp().translateY.set(4 * display_ctrl.get_ctrl_scale())

        return display_ctrl

    def create_ik_fk(self, attach_node: pm.PyNode | None, scale: float) -> control.Control:
        ...
        if attach_node:
            ...
            ikfk_ctrl.get_modify_grp().translateY.set(3 * ikfk_ctrl.get_ctrl_scale())
...
class Module:  # pylint: disable=too-few-public-methods,too-many-instance-attributes
    ...
    def __init__(
        ...
        **legacy_kwargs: Any,
        ...
        if legacy_kwargs:
            raise ValueError(f"Unexpected arguments for Module: {tuple(legacy_kwargs.keys())}")

        self.top_group = pm.group(n=f"{prefix}Module_GRP", em=True)

        self.controls_group = pm.group(n=f"{prefix}Controls_GRP", em=True, p=self.top_group)
        self.secondary_controls_group = pm.group(
            n=f"{prefix}secondaryControls_GRP", em=True, p=self.top_group
        )
        self.joints_group = pm.group(n=f"{prefix}Joints_GRP", em=True, p=self.top_group)
        self.parts_group = pm.group(n=f"{prefix}Parts_GRP", em=True, p=self.top_group)

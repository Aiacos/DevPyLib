...
class FootRoll:
    ...
    def __init__(
        ...
        for ball_joint in ball_joint_list:
            ...
            ball_ik = pm.ikHandle(n=f"{prefix}Ball_IKH", sj=ball_joint_parent, ee=ball_joint)[0]
        ...
        for toe_joint, ball_joint in zip(toe_end_joint_list, ball_joint_list, strict=False):
            ...
            toe_ik = pm.ikHandle(n=f"{prefix}Fng_IKH", sj=ball_joint, ee=toe_joint)[0]
    ...
    def roll_groups(self, front_roll_loc, back_roll_loc, inner_roll_loc, outer_roll_loc):
        ...
        if all(pm.objExists(loc) for loc in locators):
            ...
            front_roll_grp = pm.group(self.tippy_toe_grp, n=f"{name.remove_suffix(front_loc)}_GRP")
            back_roll_grp = pm.group(front_roll_grp, n=f"{name.remove_suffix(back_loc)}_GRP")
            inner_roll_grp = pm.group(back_roll_grp, n=f"{name.remove_suffix(inner_loc)}_GRP")
            outer_roll_grp = pm.group(inner_roll_grp, n=f"{name.remove_suffix(outer_loc)}_GRP")
    ...
    def move_grp(self):
        ...
        if self.outer_roll_grp:
            group = pm.group(self.outer_roll_grp, n=f"{self.prefix_joint_ankle}Move_GRP")
        else:
            group = pm.group(self.tippy_toe_grp, n=f"{self.prefix_joint_ankle}Move_GRP")

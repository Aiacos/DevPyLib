...
class USDCharacterBuild(object):
    ...
    def __init__(
        ...
        self.create_usd_stage_node = bifrost.bf_create_node(
            self.bifrost_shape, "BifrostGraph,USD::Stage,create_usd_stage"
        )
        self.time_node = bifrost.bf_create_node(
            self.bifrost_shape, "BifrostGraph,Core::Time,time"
        )
        ...
        external_usd_input = bifrost.bf_add_output_port(
            self.bifrost_shape, "/input", "external_usd", "string"
        )
        self.file_ext_input = bifrost.bf_add_output_port(
            self.bifrost_shape, "/input", "file_extension", "string"
        )

        self.type_manager_compound = bifrost_util_nodes.file_type_manager(
            self.bifrost_shape, self.create_usd_stage_node
        )

        # Create Preview Compound
        self.preview_compound = bifrost_util_nodes.build_preview_compound(
            self.bifrost_shape
        )

        # Make external usd file available
        bifrost.bf_connect(
            self.bifrost_shape, external_usd_input, self.preview_compound + ".file"
        )
    ...
    def connect_output(self):
        ...
        id_array_node = bifrost.bf_rename_node(
            self.bifrost_shape, id_array_node, "id_build_array"
        )

        layer_array_node = bifrost.bf_create_node(
            self.bifrost_shape, "BifrostGraph,Core::Array,build_array"
        )
        layer_array_node = bifrost.bf_rename_node(
            self.bifrost_shape, layer_array_node, "layer_build_array"
        )
        ...
        bifrost.bf_connect(
            self.bifrost_shape, id_array_node + ".array", "output.id_array"
        )  # "/build_array1.array" ".id_array"
        bifrost.bf_connect(
            self.bifrost_shape,
            layer_array_node + ".array",
            self.preview_compound + ".sublayers",
        )  # "/layer_build_array.array" "/Preview_Compound.sublayers"
    ...
    def add_stage_custom_layer_data(self, data, product):
        ...
        bifrost.bf_connect(
            self.bifrost_shape,
            self.create_usd_stage_node + ".stage",
    ...
    def add_product(
        ...
        (
            add_to_stage_node,
            save_usd_stage_node,
            set_stage_time_code_node,
            add_custom_layer_data_node,
        ) = self.create_default_usd_stage(product_name, data=custom_layer_data)
        node_list.extend(
            [
                add_to_stage_node,
                save_usd_stage_node,
        ...
        if self.single_usd:
            add_to_stage_compound = bifrost.bf_create_compound(
                self.bifrost_shape, [self.add_to_stage_node]
            )
            bifrost.bf_feedback_port(
                self.bifrost_shape, add_to_stage_compound, "out_stage", "stage"
            )
            ...
            ) = bifrost_util_nodes.build_name(
                self.bifrost_shape,
                self.time_node,
                product=product_name,
                extension_format=self.file_ext,
                skip_frame=False,
            )
            bifrost.bf_connect(
                self.bifrost_shape,
                string_join_node + ".joined",
                save_usd_stage_node + ".file",
            )
            bifrost.bf_connect(
                self.bifrost_shape, self.file_ext_input, extension_value_node + ".value"
            )
        ...
        # Block unwanted data
        iterator_node = self.create_block_loop(
            add_to_stage_node, set_stage_time_code_node
        )
        node_list.append(iterator_node)
        ################

        # Create Payload and Reference Compound
        reference_compound = bifrost_util_nodes.build_referece_peyload(
            self.bifrost_shape, layer_name=product_name, is_payload="0"
        )
        payload_compound = bifrost_util_nodes.build_referece_peyload(
            self.bifrost_shape, layer_name=product_name, is_payload="1"
        )
        bifrost.bf_connect(
            self.bifrost_shape,
            save_usd_stage_node + ".out_stage",
            payload_compound + ".stage",
        )
        bifrost.bf_connect(
            self.bifrost_shape,
            payload_compound + ".out_stage",
            reference_compound + ".stage",
        )
        # bifrost.bf_connect(self.bifrost_shape, reference_compound + '.out_stage', self.preview_compound + '.stage')
        node_list.extend(
            [reference_compound.replace("/", ""), payload_compound.replace("/", "")]
        )

        array_node = bifrost.bf_create_node(
            self.bifrost_shape, "BifrostGraph,Core::Array,build_array"
        )
        node_list.append(array_node)

        id_payload_output = bifrost.bf_add_input_port(
            self.bifrost_shape, array_node, "id", "long"
        )
        id_reference_output = bifrost.bf_add_input_port(
            self.bifrost_shape, array_node, "id", "long"
        )

        bifrost.bf_connect(
            self.bifrost_shape, payload_compound + ".id", id_payload_output
        )
        bifrost.bf_connect(
            self.bifrost_shape, reference_compound + ".id", id_reference_output
        )

        # Converto to layer
        get_root_layer_node = bifrost.bf_create_node(
            self.bifrost_shape, "BifrostGraph,USD::Layer,get_root_layer"
        )
        node_list.append(get_root_layer_node)
        bifrost.bf_connect(
            self.bifrost_shape,
            reference_compound + ".out_stage",
            get_root_layer_node + ".stage",
        )

        product_compound = bifrost.bf_create_compound(
            self.bifrost_shape,
            compound_node_list=node_list,
            compound_name=product_name + "_compound",
        )
        product_layer_output = bifrost.bf_add_input_port(
            self.bifrost_shape, product_compound + "/output", "layer", "auto"
        )
        bifrost.bf_connect(
            self.bifrost_shape,
            product_compound + "/" + get_root_layer_node + ".layer",
        ...
        bifrost.bf_connect(
            self.bifrost_shape,
            product_compound + "/" + array_node + ".array",
            product_compound + "." + product_name + "_id_array",
        )

        product_id_output = bifrost.bf_add_input_port(
            self.bifrost_shape,
            self.id_array_node,
            product_name + "_id_array",
            "array<long>",
        ...
        array_layer_output = bifrost.bf_add_input_port(
            self.bifrost_shape,
            self.layer_array_node,
            product_name + "_layer_array",
            "BifrostUsd::Layer",
        )
        bifrost.bf_connect(
            self.bifrost_shape, product_compound + "." + "layer", array_layer_output
        )
    ...
    def create_default_usd_stage(self, product, data):
        ...
        bifrost.bf_add_output_port(
            self.bifrost_shape, "input", product + "_file", "string"
        )  # node, port_name, port_type
        bifrost.bf_add_output_port(
            self.bifrost_shape, "input", "save_" + product + "_to_file", "bool"
        )

        # Nodes Creation
        add_to_stage_node = bifrost.bf_create_node(
            self.bifrost_shape, "BifrostGraph,USD::Stage,add_to_stage"
        )
        stage_time_code_node = bifrost.bf_create_node(
            self.bifrost_shape, "BifrostGraph,USD::Stage,set_stage_time_code"
        )
        save_usd_stage_node = bifrost.bf_create_node(
            self.bifrost_shape, "BifrostGraph,USD::Stage,save_usd_stage"
        )

        add_custom_layer_data_node = self.add_stage_custom_layer_data(
        ...
        bifrost.bf_connect(
            self.bifrost_shape,
            self.create_usd_stage_node + ".stage",
            add_to_stage_node + ".stage",
        )
        bifrost.bf_connect(
            self.bifrost_shape,
            add_to_stage_node + ".out_stage",
            stage_time_code_node + ".stage",
        )
        bifrost.bf_connect(
            self.bifrost_shape,
            stage_time_code_node + ".out_stage",
            save_usd_stage_node + ".stage",
        )
        ...
        bifrost.bf_connect(
            self.bifrost_shape,
            add_custom_layer_data_node + ".out_stage",
            save_usd_stage_node + ".stage",
        )

        bifrost.bf_connect(
            self.bifrost_shape,
            "input." + "save_" + product + "_to_file",
            save_usd_stage_node + ".enable",
        )

        bifrost.bf_connect(
            self.bifrost_shape, "input.start_frame", stage_time_code_node + ".start"
        )
        bifrost.bf_connect(
            self.bifrost_shape, "input.end_frame", stage_time_code_node + ".end"
        )
        bifrost.bf_connect(
            self.bifrost_shape, "input.layer_index", add_to_stage_node + ".layer_index"
        )
    ...
    def create_prim(self, name_dict, prim_type="Xform", specifier_over=False):
        ...
        if node_name in graph_node_list:
            ...
            new_node = bifrost.bf_create_node(
                self.bifrost_shape, "BifrostGraph,USD::Prim,define_usd_prim"
            )
            bifrost.bf_set_node_property(
                self.bifrost_shape, new_node, "path", "/" + name_dict["short_name"]
            )
            new_node = bifrost.bf_rename_node(
                self.bifrost_shape,
                new_node,
                name_dict["long_name"] + "_define_usd_prim",
            )

            bifrost.bf_set_node_property(
                self.bifrost_shape, node_name, "type", prim_type
            )

            if specifier_over:
                bifrost.bf_set_node_property(
                    self.bifrost_shape, node_name, "specifier", "1"
                )  # Set Over
    ...
    def add_undeformed_mesh_transform(self, mesh_name):
        ...
        define_usd_transform_node = bifrost.bf_create_node(
            self.bifrost_shape, "BifrostGraph,USD::Attribute,define_usd_transform"
        )
        cmds.vnnCompound(
            self.bifrost_shape, "/" + define_usd_transform_node, setIsReferenced=False
        )

        bifrost.bf_add_output_port(
            self.bifrost_shape, define_usd_transform_node + "/input", "frame", "float"
        )
        bifrost.bf_add_output_port(
            self.bifrost_shape,
            define_usd_transform_node + "/input",
            "use_frame",
            "bool",
        )
        ...
        bifrost.bf_connect(
            self.bifrost_shape,
            define_usd_transform_node + ".frame",
            define_usd_transform_node + "/define_usd_attribute2" + ".frame",
        )
        bifrost.bf_connect(
            self.bifrost_shape,
            define_usd_transform_node + ".use_frame",
        ...
        define_usd_transform_node = bifrost.bf_rename_node(
            self.bifrost_shape, define_usd_transform_node, node_name
        )
    ...
    def add_undeformed_mesh(self, mesh_name):
        ...
        # Create nodes
        define_mesh_node = bifrost.bf_create_node(
            self.bifrost_shape, "BifrostGraph,USD::Prim,define_usd_mesh"
        )

        bifrost.bf_set_node_property(
            self.bifrost_shape, define_mesh_node, "path", "/" + name_dict["short_name"]
        )
        define_mesh_node = bifrost.bf_rename_node(
            self.bifrost_shape,
            define_mesh_node,
    ...
    def add_xform(self, obj, obj_node=None):
        ...
        if connected_node_list == None or not (node in connected_node_list):
            ...
            bifrost.bf_add_output_port(
                self.bifrost_shape,
                "input",
                name_dict["short_name"] + "_translate",
                "Math::float3",
            )
            bifrost.bf_add_output_port(
                self.bifrost_shape,
                "input",
                name_dict["short_name"] + "_rotate",
                "Math::float3",
            )
            bifrost.bf_add_output_port(
                self.bifrost_shape,
                "input",
                name_dict["short_name"] + "_scale",
                "Math::float3",
            )

            # Create nodes
            define_attribute_node = self.add_undeformed_mesh_transform(
                name_dict["long_name"]
            )
            bifrost.bf_set_node_property(
                self.bifrost_shape, define_attribute_node, "use_frame", "1"
            )

            # Connect
            bifrost.bf_connect(
                self.bifrost_shape,
                self.time_node + ".frame",
                define_attribute_node + ".frame",
            )

            bifrost.bf_connect(
                self.bifrost_shape,
                "input." + name_dict["short_name"] + "_translate",
                define_attribute_node + ".translation",
            )
            bifrost.bf_connect(
                self.bifrost_shape,
                "input." + name_dict["short_name"] + "_rotate",
                define_attribute_node + ".rotation",
            )
            bifrost.bf_connect(
                self.bifrost_shape,
                "input." + name_dict["short_name"] + "_scale",
            ...
            bifrost.bf_connect(
                self.bifrost_shape,
                define_attribute_node + ".attribute_definitions",
                input_port,
            )
    ...
    def add_mesh(self, mesh_name, add_to_stage_node):
        ...
        mesh_out_port = bifrost.bf_list_all_port(
            self.bifrost_shape, input_mesh_node, input_port=False, output_port=True
        )[-1]

        bifrost.bf_connect(
            self.bifrost_shape, mesh_out_port, define_mesh_node + ".mesh"
        )
        bifrost.bf_connect(
            self.bifrost_shape, self.time_node + ".frame", define_mesh_node + ".frame"
        )
    ...
    def recursive_build_usd_graph(self, obj_data, node, add_to_stage_node):
        ...
        if cmds.objExists(obj_data["full_path"]):
            ...
            parent = cmds.listRelatives(obj_data["full_path"], p=True, fullPath=True)[
                -1
            ]
            ...
            connection_check = (
                cmds.connectionInfo(obj_data["full_path"] + ".tx", isDestination=True)
                or cmds.connectionInfo(
                    obj_data["full_path"] + ".ty", isDestination=True
                )
                or cmds.connectionInfo(
                    obj_data["full_path"] + ".tz", isDestination=True
                )
                or cmds.connectionInfo(
                    obj_data["full_path"] + ".rx", isDestination=True
                )
                or cmds.connectionInfo(
                    obj_data["full_path"] + ".ry", isDestination=True
                )
                or cmds.connectionInfo(
                    obj_data["full_path"] + ".rz", isDestination=True
                )
                or cmds.connectionInfo(
                    obj_data["full_path"] + ".sx", isDestination=True
                )
                or cmds.connectionInfo(
                    obj_data["full_path"] + ".sy", isDestination=True
                )
                or cmds.connectionInfo(
            ...
            if parent_obj["short_name"] == self.root_node["short_name"]:
                ...
                connection_check = (
                    cmds.connectionInfo(
                        parent_obj["full_path"] + ".tx", isDestination=True
                    )
                    or cmds.connectionInfo(
                        obj_data["full_path"] + ".ty", isDestination=True
                    )
                ...
                if connected_node_list == None or not (node in connected_node_list):
                    input_port = bifrost.bf_add_input_port(
                        self.bifrost_shape,
                        new_node,
                        "children.prim_definition",
                        "auto",
                        "children",
                    )
                    bifrost.bf_connect(
                        self.bifrost_shape, node + ".prim_definition", input_port
                    )
                ...
                if connected_node_list == None or not (node in connected_node_list):
                    ...
                    if (
                        ...
                        input_port = bifrost.bf_add_input_port(
                            self.bifrost_shape,
                            new_node,
                            "children.mesh_definition",
                            "auto",
                            "children",
                        )
                        bifrost.bf_connect(
                            self.bifrost_shape, node + ".mesh_definition", input_port
                        )
                    else:
                        input_port = bifrost.bf_add_input_port(
                            self.bifrost_shape,
                    ...
                    self.recursive_build_usd_graph(
                        parent_obj, new_node, add_to_stage_node
                    )
                    self.recursive_node_list.append(new_node)
    ...
    def add_block_attribute(self, attr_name, node_name="", prim_path="", parent=""):
        ...
        """
        block_attribute_node = bifrost.bf_create_node(
            self.bifrost_shape, "BifrostGraph,USD::Attribute,block_attribute", parent
        )
        bifrost.bf_set_node_property(
            self.bifrost_shape, parent + "/" + block_attribute_node, "name", attr_name
        )

        if prim_path != "":
            bifrost.bf_set_node_property(
                self.bifrost_shape,
                parent + "/" + block_attribute_node,
                "prim_path",
                prim_path,
            )
    ...
    def create_block_loop(self, add_to_stage_node, set_stage_time_code_node):
        ...
        """
        # Create nodes
        get_prim_children_node = bifrost.bf_create_node(
            self.bifrost_shape, "BifrostGraph,USD::Prim,get_prim_children"
        )
        get_prim_path_node = bifrost.bf_create_node(
            self.bifrost_shape, "BifrostGraph,USD::Prim,get_prim_path"
        )
        array_size_node = bifrost.bf_create_node(
            self.bifrost_shape, "BifrostGraph,Core::Array,array_size"
        )
        iterator_node = bifrost.bf_create_node(
            self.bifrost_shape, "BifrostGraph,Core::Iterators,iterate"
        )

        get_from_array_node = bifrost.bf_create_node(
            self.bifrost_shape,
            "BifrostGraph,Core::Array,get_from_array",
            parent="/" + iterator_node,
        )
        face_vertex_counts_block_node = self.add_block_attribute(
            "faceVertexCounts", "faceVertexCounts", parent="/" + iterator_node
        )
        face_vertex_indices_block_node = self.add_block_attribute(
            "faceVertexIndices", "faceVertexIndices", parent="/" + iterator_node
        )
        primvars_st_block_node = self.add_block_attribute(
            "primvars:st", "primvars_st", parent="/" + iterator_node
        )
        primvars_st_indices_block_node = self.add_block_attribute(
            "primvars:st:indices", "primvars_st_indices", parent="/" + iterator_node
        )

        # Add ports
        bifrost.bf_add_output_port(
            self.bifrost_shape, iterator_node + "/input", "out_stage", "auto"
        )
        bifrost.bf_add_output_port(
            self.bifrost_shape, iterator_node + "/input", "path", "auto"
        )
        bifrost.bf_add_input_port(
            self.bifrost_shape, iterator_node + "/output", "out_stage1", "auto"
        )
        ...
        bifrost.bf_connect(
            self.bifrost_shape,
            add_to_stage_node + ".out_stage",
            get_prim_children_node + ".stage",
        )
        bifrost.bf_connect(
            self.bifrost_shape,
            get_prim_children_node + ".children",
            get_prim_path_node + ".prim",
        )
        bifrost.bf_connect(
            self.bifrost_shape, get_prim_path_node + ".path", array_size_node + ".array"
        )
        bifrost.bf_connect(
            self.bifrost_shape,
            array_size_node + ".size",
            iterator_node + ".max_iterations",
        )

        bifrost.bf_connect(
            self.bifrost_shape,
            add_to_stage_node + ".out_stage",
            iterator_node + ".out_stage",
        )
        bifrost.bf_connect(
            self.bifrost_shape, get_prim_path_node + ".path", iterator_node + ".path"
        )
        ...
        bifrost.bf_connect(
            self.bifrost_shape,
            iterator_node + "/" + face_vertex_counts_block_node + ".out_stage",
            iterator_node + "/" + face_vertex_indices_block_node + ".stage",
        )
        bifrost.bf_connect(
            self.bifrost_shape,
            iterator_node + "/" + face_vertex_indices_block_node + ".out_stage",
            iterator_node + "/" + primvars_st_block_node + ".stage",
        )
        bifrost.bf_connect(
            self.bifrost_shape,
            iterator_node + "/" + primvars_st_block_node + ".out_stage",
            iterator_node + "/" + primvars_st_indices_block_node + ".stage",
        )
        bifrost.bf_connect(
            self.bifrost_shape,
            iterator_node + "/" + primvars_st_indices_block_node + ".out_stage",
            iterator_node + ".out_stage1",
        )

        bifrost.bf_connect(
            self.bifrost_shape,
            iterator_node + "/" + get_from_array_node + ".value",
        ...
        bifrost.bf_connect(
            self.bifrost_shape,
            iterator_node + ".out_stage1",
            set_stage_time_code_node + ".stage",
        )

        # Settings
        bifrost.bf_sequence_port(
            self.bifrost_shape, "/" + iterator_node, "out_stage", "out_stage1"
        )
        bifrost.bf_set_node_property(
            self.bifrost_shape,
            get_prim_children_node,
            "prim_path",
            "/" + self.root_node["short_name"],
        )
        bifrost.bf_set_node_property(
            self.bifrost_shape, get_prim_children_node, "descendant_mode", "3"
        )

...
def file_type_manager(bifrost_shape, create_stage_node):
    ...
    type_manager_compund = bifrost.bf_create_compound(
        bifrost_shape, compound_node_list=node_list, compound_name="extension_Compound", parent="/"
    )
...
def add_custom_layer_data(bifrost_shape, data, custom_layer_data_node, product=""):
    ...
    bifrost.bf_connect(
        bifrost_shape, node_list[-1] + ".out_object", custom_layer_data_node + ".value"
    )
    custom_layer_data_compound = bifrost.bf_create_compound(
        bifrost_shape,
        compound_node_list=node_list,
        compound_name=product + "_custom_layer_data_Compound",
        parent="/",
    )
...
def set_stage_property(bifrost_shape, key, value="", source=None):
    ...
    # Create nodes
    set_property_node = bifrost.bf_create_node(
    ...
    if value != "" and value != source:
        if isinstance(value, int):
            value_value_node = bifrost.bf_create_node(
                bifrost_shape, "BifrostGraph,Core::Constants,int"
            )
        else:
            value_value_node = bifrost.bf_create_node(
                bifrost_shape, "BifrostGraph,Core::Constants,string"
            )
    ...
    if value != "" and value != source:
        bifrost.bf_connect(
            bifrost_shape, value_value_node + ".output", set_property_node + ".value"
        )
    else:
...
def build_name(bifrost_shape, time_node, product="", extension_format="usdc", skip_frame=False):
    ...
    string_join_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::String,string_join"
    )
    build_array_node = bifrost.bf_create_node(bifrost_shape, "BifrostGraph,Core::Array,build_array")
    extension_value_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::Constants,string"
    )
    if not skip_frame:
        number_to_string_node = bifrost.bf_create_node(
            bifrost_shape, "BifrostGraph,Core::String,number_to_string"
        )
        frame_value_node = bifrost.bf_create_node(
            bifrost_shape, "BifrostGraph,Core::Constants,string"
        )
    ...
    if not skip_frame:
        ...
        bifrost.bf_connect(
            bifrost_shape, number_to_string_node + ".string", frame_value_node + ".value"
        )

    bifrost.bf_connect(
        bifrost_shape, "input." + product + "_file", build_array_node + "." + product + "_file"
    )
    if not skip_frame:
        bifrost.bf_connect(
            bifrost_shape, frame_value_node + ".output", build_array_node + ".output"
        )
    bifrost.bf_connect(
        bifrost_shape, extension_value_node + ".output", build_array_node + ".output1"
    )

    bifrost.bf_connect(bifrost_shape, build_array_node + ".array", string_join_node + ".strings")

    # Rename Nodes
    extension_value_node = bifrost.bf_rename_node(
        bifrost_shape, extension_value_node, "extension_value"
    )
...
def build_referece_peyload(
    ...
    if use_reference:
        if is_payload == "0":
            compound = bifrost.bf_create_node(
                bifrost_shape, "BifrostGraph,EDI::Compounds,Reference_Compound"
            )
        else:
            compound = bifrost.bf_create_node(
                bifrost_shape, "BifrostGraph,EDI::Compounds,Payload_Compound"
            )
        ...
        compound = bifrost.bf_create_compound(
            bifrost_shape, compound_node_list=[], compound_name=name + "_Compound", parent="/"
        )

        # Create nodes
        create_stage_node = bifrost.bf_create_node(
            bifrost_shape, "BifrostGraph,USD::Stage,create_usd_stage", compound
        )
        add_to_stage_node = bifrost.bf_create_node(
            bifrost_shape, "BifrostGraph,USD::Stage,add_to_stage", compound
        )
        get_root_layer_node = bifrost.bf_create_node(
            bifrost_shape, "BifrostGraph,USD::Layer,get_root_layer", compound
        )
        add_payload_prim_node = bifrost.bf_create_node(
            bifrost_shape, "BifrostGraph,USD::Prim,add_payload_prim", compound
        )
        add_reference_prim_node = bifrost.bf_create_node(
            bifrost_shape, "BifrostGraph,USD::Prim,add_reference_prim", compound
        )
        if_node = bifrost.bf_create_node(bifrost_shape, "BifrostGraph,Core::Logic,if", compound)
        define_usd_prim_node = bifrost.bf_create_node(
            bifrost_shape, "BifrostGraph,USD::Prim,define_usd_prim", compound
        )
        send_stage_to_cache_node = bifrost.bf_create_node(
        ...
        bifrost.bf_set_node_property(
            bifrost_shape, compound + "/" + create_stage_node, "layer", layer_name
        )
        bifrost.bf_set_node_property(
            bifrost_shape, compound + "/" + define_usd_prim_node, "path", "/root"
        )
        bifrost.bf_set_node_property(
            bifrost_shape, compound + "/" + add_payload_prim_node, "prim_path", "/root"
        )
        bifrost.bf_set_node_property(
            bifrost_shape, compound + "/" + add_payload_prim_node, "payload_prim_path", "/root"
        )

        bifrost.bf_set_node_property(
            bifrost_shape, compound + "/" + add_reference_prim_node, "prim_path", "/root"
        )
        bifrost.bf_set_node_property(
            bifrost_shape, compound + "/" + add_reference_prim_node, "reference_prim_path", "/root"
        )
        bifrost.bf_set_node_property(
            bifrost_shape, compound + "/" + if_node, "condition", is_payload
        )

        # Create Ports
        compound = compound.replace("/", "")
        prim_definition_input_port = bifrost.bf_add_input_port(
            bifrost_shape,
            compound + "/" + add_to_stage_node,
            "prim_definitions.prim_definition",
            "auto",
            "prim_definitions",
        )

        compound_stage_input = bifrost.bf_add_output_port(
            bifrost_shape, compound + "/input", "stage", "BifrostUsd::Stage"
        )
        compound_variant_set_name_input = bifrost.bf_add_output_port(
            bifrost_shape, compound + "/input", "variant_set_name", "string"
        )
        compound_variant_name_input = bifrost.bf_add_output_port(
            bifrost_shape, compound + "/input", "variant_name", "string"
        )
        compound_payload_input = bifrost.bf_add_output_port(
            bifrost_shape, compound + "/input", "Payload", "bool"
        )

        compound_stage_output = bifrost.bf_add_input_port(
            bifrost_shape, compound + "/output", "out_stage", "BifrostUsd::Stage"
        )
        ...
        bifrost.bf_connect(
            bifrost_shape,
            compound + "/" + create_stage_node + ".stage",
            compound + "/" + add_to_stage_node + ".stage",
        )
        bifrost.bf_connect(
            bifrost_shape,
            compound + "/" + define_usd_prim_node + ".prim_definition",
            prim_definition_input_port,
        )
        bifrost.bf_connect(
            bifrost_shape,
            compound + "/" + add_to_stage_node + ".out_stage",
        ...
        bifrost.bf_connect(
            bifrost_shape,
            compound + "/" + get_root_layer_node + ".layer",
        ...
        bifrost.bf_connect(
            bifrost_shape,
            compound + "/" + compound_stage_input,
            compound + "/" + get_root_layer_node + ".stage",
        )
        bifrost.bf_connect(
            bifrost_shape,
            compound + "/" + compound_variant_set_name_input,
            compound + "/" + define_usd_prim_node + ".variant_set_name",
        )
        bifrost.bf_connect(
            bifrost_shape,
            compound + "/" + compound_variant_name_input,
            compound + "/" + define_usd_prim_node + ".variant_name",
        )
        bifrost.bf_connect(
            bifrost_shape,
            compound + "/" + compound_payload_input,
            compound + "/" + if_node + ".condition",
        )

        bifrost.bf_connect(
            bifrost_shape,
            compound + "/" + add_payload_prim_node + ".out_stage",
            compound + "/" + if_node + ".true_case",
        )
        bifrost.bf_connect(
            bifrost_shape,
            compound + "/" + add_reference_prim_node + ".out_stage",
            compound + "/" + if_node + ".false_case",
        )

        bifrost.bf_connect(
            bifrost_shape,
            compound + "/" + if_node + ".output",
            compound + "/" + compound_stage_output,
        )

        bifrost.bf_connect(
            bifrost_shape,
            compound + "/" + if_node + ".output",
            compound + "/" + send_stage_to_cache_node + ".stage",
        )
        bifrost.bf_connect(
            bifrost_shape,
            compound + "/" + send_stage_to_cache_node + ".id",
            compound + "/" + id_output,
        )
        ...
        if use_variant:
            bifrost.bf_set_node_property(
                bifrost_shape,
                compound + "/" + variant_selector_node,
                "variant_set_name",
                "modeling",
            )
            bifrost.bf_set_node_property(
                bifrost_shape, compound + "/" + variant_selector_node, "selection", "default"
            )
            ...
            bifrost.bf_connect(
                bifrost_shape,
                compound + "/" + variant_selector_node + ".variant_set_definition",
                compound
                + "/"
                + define_usd_prim_node
                + ".variant_set_definitions.variant_set_definition",
            )
...
def build_preview_compound(
    ...
    """
    compound = bifrost.bf_create_compound(
        bifrost_shape, compound_node_list=[], compound_name="Preview_Compound", parent="/"
    )
    ...
    create_layer_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,USD::Stage,create_usd_stage", compound
    )
    send_stage_to_cache_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,USD::Stage,send_stage_to_cache", compound
    ...
    working_value_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::Constants,string", compound
    )
    step_buffer_value_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::Constants,string", compound
    )
    build_string_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::String,build_string", compound
    )
    custom_layer_data_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,USD::Stage,set_stage_custom_layer_data", compound
    )
    get_root_layer_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,USD::Layer,get_root_layer", compound
    )

    split_string_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::String,split_string", compound
    )
    array_size_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::Array,array_size", compound
    )
    for_each_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::Iterators,for_each", compound
    )

    get_from_array_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::Array,get_from_array", compound + "/" + for_each_node
    )
    open_layer_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,USD::Layer,open_layer", compound + "/" + for_each_node
    )

    equal_layer_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::Logic,equal", compound + "/" + for_each_node
    )
    if_layer_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::Logic,if", compound + "/" + for_each_node
    )

    ## Custom layer Data
    layer_type_property_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::Object,set_property", compound
    )
    layer_type_value_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::Constants,string", compound
    )

    step_property_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::Object,set_property", compound
    )
    step_value_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::String,string_upper", compound
    )

    usd_products_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::Object,set_property", compound
    )
    ...
    # Set Property
    bifrost.bf_set_node_property(
        bifrost_shape, compound + "/" + for_each_node + "/" + open_layer_node, "read_only", "1"
    )
    bifrost.bf_set_node_property(
        bifrost_shape, compound + "/" + layer_type_property_node, "key", "layer_type"
    )
    bifrost.bf_set_node_property(
        bifrost_shape, compound + "/" + layer_type_value_node, "value", "Step"
    )
    ...
    bifrost.bf_set_node_property(
        bifrost_shape, compound + "/" + usd_products_node, "key", "usd_products"
    )

    bifrost.bf_set_node_property(
        bifrost_shape, compound + "/" + create_layer_node, "layer", working_layer_name
    )  # "Preview_layer"
    bifrost.bf_set_node_property(
        bifrost_shape, compound + "/" + create_stage_node, "layer", "Preview_Stage"
    )

    bifrost.bf_set_node_property(
        bifrost_shape, compound + "/" + working_value_node, "value", "WORKING_"
    )

    bifrost.bf_set_node_property(
        bifrost_shape, compound + "/" + split_string_node, "separator", ";"
    )

    # Create Ports
    compound = compound.replace("/", "")
    _ = bifrost.bf_add_output_port(
        bifrost_shape, compound + "/input", "sublayers", "array<BifrostUsd::Layer>"
    )
    compound_file_input = bifrost.bf_add_output_port(
        bifrost_shape, compound + "/input", "file", "string"
    )
    ...
    _ = bifrost.bf_add_input_port(
        bifrost_shape,
        compound + "/" + create_layer_node,
        "sublayers.sublayers",
        "array<BifrostUsd::Layer>",
    )
    _ = bifrost.bf_add_input_port(
        bifrost_shape, compound + "/" + create_stage_node, "sublayers.layer", "auto"
    )
    ...
    _ = bifrost.bf_add_output_port(
        bifrost_shape, compound + "/" + for_each_node + "/input", "split", "auto"
    )
    _ = bifrost.bf_add_input_port(
        bifrost_shape, compound + "/" + for_each_node + "/output", "layer", "auto"
    )
    ...
    bifrost.bf_connect(
        bifrost_shape,
        compound + ".sublayers",
        compound + "/" + create_layer_node + ".sublayers.sublayers",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + get_root_layer_node + ".layer",
        compound + "/" + for_each_node + ".working_layer",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + custom_layer_data_node + ".out_stage",
        compound + "/" + get_root_layer_node + ".stage",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + create_layer_node + ".stage",
        compound + "/" + custom_layer_data_node + ".stage",
    )

    bifrost.bf_connect(
        bifrost_shape, compound + ".step", compound + "/" + step_buffer_value_node + ".value"
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + step_buffer_value_node + ".output",
        compound + "/" + build_string_node + ".strings.output1",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + working_value_node + ".output",
        compound + "/" + build_string_node + ".strings.output",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + build_string_node + ".joined",
        compound + "/" + create_layer_node + ".layer",
    )

    ## Custom Data
    bifrost.bf_connect(
        bifrost_shape, compound + ".custom_data", compound + "/" + usd_products_node + ".value"
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + layer_type_property_node + ".out_object",
        compound + "/" + step_property_node + ".object",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + step_property_node + ".out_object",
        compound + "/" + usd_products_node + ".object",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + usd_products_node + ".out_object",
        compound + "/" + custom_layer_data_node + ".value",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + layer_type_value_node + ".output",
        compound + "/" + layer_type_property_node + ".value",
    )
    bifrost.bf_connect(
        bifrost_shape, compound + ".step", compound + "/" + step_value_node + ".string"
    )
    bifrost.bf_connect(
        bifrost_shape, compound + ".step", compound + "/" + step_property_node + ".value"
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + step_value_node + ".upper_case",
        compound + "/" + step_buffer_value_node + ".value",
    )
    ...
    bifrost.bf_connect(
        bifrost_shape, compound + "/" + send_stage_to_cache_node + ".id", compound + "/" + id_output
    )
    ...
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + compound_file_input,
        compound + "/" + split_string_node + ".string",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + split_string_node + ".split",
        compound + "/" + array_size_node + ".array",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + array_size_node + ".size",
        compound + "/" + for_each_node + ".max_iterations",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + split_string_node + ".split",
        compound + "/" + for_each_node + ".split",
    )

    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + for_each_node + ".split",
        compound + "/" + for_each_node + "/" + get_from_array_node + ".array",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + for_each_node + ".current_index",
        compound + "/" + for_each_node + "/" + get_from_array_node + ".index",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + for_each_node + "/" + get_from_array_node + ".value",
        compound + "/" + for_each_node + "/" + open_layer_node + ".file",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + for_each_node + "/" + if_layer_node + ".output",
        compound + "/" + for_each_node + ".layer",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + for_each_node + "/" + get_from_array_node + ".value",
        compound + "/" + for_each_node + "/" + equal_layer_node + ".first",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + for_each_node + "/" + equal_layer_node + ".output",
        compound + "/" + for_each_node + "/" + if_layer_node + ".condition",
    )

    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + for_each_node + ".working_layer",
        compound + "/" + for_each_node + "/" + if_layer_node + ".true_case",
    )
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + for_each_node + "/" + open_layer_node + ".layer",
        compound + "/" + for_each_node + "/" + if_layer_node + ".false_case",
    )

    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + for_each_node + ".layer",
        compound + "/" + create_stage_node + ".sublayers.external_layers",
    )

    bifrost.bf_set_node_property(
        bifrost_shape,
        compound + "/" + for_each_node + "/" + equal_layer_node,
        "second",
        placeholder,
    )
...
def get_data_from_layer(bifrost_shape, layer_node, product_name):
    ...
    """
    # Create Nodes
    get_layer_id_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,USD::Layer,get_layer_identifier"
    )
    layer_file_path_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,USD::Layer,get_layer_file_path"
    )
    split_file_path_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::String,split_string"
    )
    get_from_array_node = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::Array,get_from_array"
    )

    product_dict_property = bifrost.bf_create_node(
        bifrost_shape, "BifrostGraph,Core::Object,set_property"
    )
    ...
    bifrost.bf_connect(
        bifrost_shape, layer_file_path_node + ".file", split_file_path_node + ".string"
    )
    bifrost.bf_connect(
        bifrost_shape, split_file_path_node + ".split", get_from_array_node + ".array"
    )
    ...
    bifrost.bf_connect(
        bifrost_shape, get_from_array_node + ".value", product_dict_property + ".key"
    )
    bifrost.bf_connect(
        bifrost_shape, path_property + ".out_object", product_dict_property + ".value"
    )

    node_list = [
        get_layer_id_node,
        layer_file_path_node,
        split_file_path_node,
        get_from_array_node,
        product_dict_property,
        name_property,
        path_property,
    ]
    compound = bifrost.bf_create_compound(
        bifrost_shape, compound_node_list=node_list, compound_name=product_name + "_data_compound"
    )
    ...
    bifrost.bf_connect(
        bifrost_shape,
        compound + "/" + product_dict_property + ".out_object",
        compound + ".out_data",
    )

    bifrost.bf_add_output_port(bifrost_shape, compound + "/input", "in_data", "auto")
    bifrost.bf_connect(
        bifrost_shape, compound + ".in_data", compound + "/" + product_dict_property + ".object"
    )

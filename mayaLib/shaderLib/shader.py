...
class TextureShader:
    ...
    def __init__(self, texture_path, geo_name, textureset_dict, single_place_node=True):
        ...
        if self.renderer == "arnold":
            self.shader = self.build_ai_standard(
                texture_path, geo_name, textureset_dict, single_place_node=True
            )
    ...
    def build_pxr_disney_bsdf(
        self, texture_path, geo_name, textureset_dict, pxr_texture_node=True, single_place_node=True
    ):
        ...
        """
        for texture_channel in textureset_dict:
            fn = texture.TexturePxrTexture(
                path=texture_path, filename=textureset_dict[texture_channel]
            )
            self.filenode_dict[texture_channel] = fn.filenode
...
class ConvertShaders:
    ...
    def reconnect_filenode(self, shader, new_shader):
        ...
        if diffuse_socket.isConnected():
            ...
            pm.connectAttr(
                diffuse_socket.listConnections(plugs=True)[-1],
                pm.PyNode(new_shader.get_shader().name() + "." + new_shader.diffuse),
                f=True,
            )
        if metallic_socket.isConnected():
            pm.connectAttr(
                metallic_socket.listConnections(plugs=True)[-1],
                pm.PyNode(new_shader.get_shader().name() + "." + new_shader.metallic),
                f=True,
            )
        if roughness_socket.isConnected():
            pm.connectAttr(
                roughness_socket.listConnections(plugs=True)[-1],
                pm.PyNode(new_shader.get_shader().name() + "." + new_shader.roughness),
                f=True,
            )
        if emission_socket.isConnected():
            pm.connectAttr(
                emission_socket.listConnections(plugs=True)[-1],
                pm.PyNode(new_shader.get_shader().name() + "." + new_shader.emission),
                f=True,
            )
        if alpha_socket.isConnected():
            ...
            try:
                pm.connectAttr(
                    alpha_socket.listConnections(plugs=True)[-1],
                    pm.PyNode(new_shader.get_shader().name() + "." + new_shader.alpha),
                    f=True,
                )
        ...
        if normal_socket.isConnected():
            pm.connectAttr(
                normal_socket.listConnections(plugs=True)[-1],
                pm.PyNode(new_shader.get_shader().name() + "." + new_shader.normal),
                f=True,
            )
